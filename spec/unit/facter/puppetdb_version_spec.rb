...
describe 'openvoxdb_version' do
  ...
  context 'when puppetdb is available' do
    before do
      allow(Facter::Core::Execution).to receive(:which).with('puppetdb').and_return('/usr/bin/puppetdb')
    end
  ...
  context 'when puppetdb is not available' do
    before do
      allow(Facter::Core::Execution).to receive(:which).with('puppetdb').and_return(nil)
    end
